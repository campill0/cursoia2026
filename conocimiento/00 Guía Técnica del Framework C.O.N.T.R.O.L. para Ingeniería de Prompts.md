...
El modelo no es un cerebro, es un motor de predicción probabilística. Funciona bajo la metáfora de **"Elías en el Búnker"**: un becario encerrado, sin internet (por defecto), con conocimiento congelado en el pasado y una tendencia a inventar para complacer ("Fluidez Engañosa").  
...
• **Tipos de Memoria:**  
    ◦ *Procedimental (Custom Instructions):* El Sistema Operativo. Fijo y constante (Quién eres tú, normas globales).  
    ◦ *Episódica (Memory):* Dinámica. Recuerdos que la IA decide guardar sobre ti.  
• **La Regla de Oro:** No valides una respuesta por lo bien que suena, sino por la evidencia que aporta,,.

\--------------------------------------------------------------------------------

1\. FASE C: CONTEXTO CURADO (Signal-to-Noise Ratio)  
...
• **Aislamiento (Sandwich Defense):** Usa delimitadores (`"""` o `###`) para separar tus datos de tus instrucciones. Evita que la IA confunda órdenes con texto a analizar.  
• **Higiene de Sesión:**  
...
2\. FASE O (1): OMNI-ROL (Ingeniería de Identidad)  
...
3\. FASE N: NORMAS Y NEGATIVAS (El Muro de Contención)  
...
4\. FASE T: TRAZA DE PENSAMIENTO (Motor Cognitivo 2026\)  
*De simular el pensamiento a calibrar motores de razonamiento.*  
**Objetivo:** Obligar al modelo a procesar antes de responder.  
• **El Nuevo Paradigma (Modelos Razonadores):** Con GPT-5.2 o Gemini 3, no micro-gestiones el proceso ("paso a paso"). Controla el **Objetivo** y los **Criterios de Calidad**, y deja que el modelo decida la ruta lógica,.  
...
5\. FASE R: REALIDAD Y RESISTENCIA (Protocolos de Verdad)  
...
6\. FASE O (2): OUTPUT Y ORGANIZACIÓN (Vibe Coding)  
...
• **Prefill (Relleno):** Escribe tú el inicio de la respuesta (ej. `...Responde en JSON: {`) para forzar la estructura.  
• **Canvas:** Usa la edición quirúrgica (sombrear texto y corregir) en lugar de regenerar todo el chat.

\--------------------------------------------------------------------------------

7\. FASE L: LOOP DE MEJORA (Recursividad)  
