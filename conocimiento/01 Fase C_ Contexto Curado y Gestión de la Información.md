...
**4\. El imperativo de la Relación Señal/Ruido:** Entender estas patologías requiere un cambio de mentalidad: debemos dejar de ver el contexto como un almacén y verlo como una transmisión de radio. 
...
• **Uso de Delimitadores (Sandwich Defense/Isolation):** Encierra los datos de contexto entre caracteres especiales para separarlos de tus órdenes. Esto evita que la IA confunda el texto que debe analizar con las instrucciones que debe seguir (y protege contra *Prompt Injection* indirecto),.  
    ◦ *Sintaxis recomendada:* Usa triples comillas (`"""`), guiones (`---`) o etiquetas XML (`<contexto> ... </contexto>`),.  
...
• **Etiquetado Semántico (Metadata Injection):** Si aportas varios documentos, etiquétalos claramente dentro del prompt. No pegues texto plano; añade cabeceras.  
    ◦ *Ejemplo:* `[Documento A: Política de Ventas 2024]` ... `[Documento B: Queja del Cliente]` ... "Compara A y B".  
...
3.1 **Cuarentena de Contexto (Context Quarantine):** Para tareas complejas o diferentes, inicia un **nuevo chat** o un nuevo hilo. No mezcles temas (ej. no pidas código Python en el mismo chat donde analizaste recetas de cocina). El "residuo" de la tarea anterior puede sesgar la nueva (Context Clash),.  
...
El Riesgo Crítico: Truncamiento Silencioso (Silent Truncation) Cuando tu conversación supera estos límites, la interfaz ejecuta una "poda automática" sin avisarte: elimina los mensajes más antiguos para hacer sitio a los nuevos.  
...
A. Custom Instructions (Memoria Procedimental / Fija) Es el equivalente a la configuración base o el sistema operativo del chat.  
...
B. Memory (Memoria Episódica / Dinámica) Es el equivalente a los recuerdos biográficos del asistente sobre ti.  
