...
import { getChatGptGuideContent } from '../lib/content';
...
const ChatGptGuide = () => {
    const rawContent = getChatGptGuideContent();
        ...
        <motion.div variants={stagger} initial="hidden" animate="show" className="space-y-12 pb-20">
            ...
            <motion.div variants={fadeUp} className="relative rounded-3xl overflow-hidden border border-surface-3 shadow-2xl">
                <div className="w-full h-40 md:h-56 bg-obsidian flex items-center justify-center relative overflow-hidden">
                    ...
                    <BookMarked className="w-24 h-24 text-surface-3 absolute -right-4 -bottom-4 opacity-50 transform rotate-12" />
                </div>
                ...
                <div className="absolute bottom-0 left-0 right-0 p-8 md:p-10 z-[3]">
                    <div className="flex items-center gap-3 mb-3">
                        <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-electric-cyan/20 text-electric-cyan border border-electric-cyan/30 tracking-wider">GUÍA</span>
                        <span className="text-[10px] font-mono text-muted/80 tracking-widest uppercase">MANUAL TÉCNICO</span>
                    </div>
                    <h1 className="text-4xl md:text-5xl font-black header-text-shadow mb-4 leading-tight">
                        <span className="text-ghost-white">Guía completa de </span><span className="text-electric-cyan">ChatGPT</span>
                    </h1>
                    <p className="text-lg text-muted max-w-2xl header-text-shadow leading-relaxed">
                        Domina la interfaz, sus funcionalidades avanzadas y la higiene de contexto para un rendimiento profesional.
                    </p>
                </div>
            </motion.div>
            ...
            <CompleteButton moduleId="chatgpt-guide" />
        </motion.div>
    );
};
