...
* **La Técnica:** En lugar de terminar el prompt con una pregunta, termínalo con el inicio de la estructura deseada.  
  * *Prompt:* "...Analiza los datos. Respuesta en JSON: {"
...
* **Utilidad No-Técnica:** Permite que la IA analice pros y contras de forma exhaustiva "en privado" y te entregue solo la conclusión profesional, evitando que el razonamiento intermedio ensucie tu documento final.  
  * *Instrucción:* "Primero piensa paso a paso dentro de etiquetas `<pensamiento>`. Luego, proporciona EXCLUSIVAMENTE el documento final dentro de etiquetas `<code>`. No escribas nada fuera de estas etiquetas".
...
### **7\. Estandarización para Automatizaciones (Make/N8N)**

Si estás integrando la IA en un flujo de trabajo (como se vio en el "Día 3" de Founderz), el output debe ser determinista para que la siguiente "bolita" del proceso lo entienda.

* **Estructura Rígida:** "Responde siempre con el mismo esquema. Si un campo no tiene datos, rellénalo con 'N/A', nunca lo omitas".  
* **Delimitadores:** Usa delimitadores claros para separar secciones si la IA genera varios contenidos a la vez (ej. un asunto de email y el cuerpo del email).  
  * *Prompt:* "Separa el asunto y el cuerpo con '\#\#\#SEPARADOR\#\#\#'".

**8\. Herramientas de Interfaz (UI): El Canvas como Mesa de Trabajo**
...
Actualización del "Resumen Táctico" (Opcional)

Te recomiendo añadir un último checkbox al final del documento para reflejar este cambio:
...
Para cerrar el círculo, añade este último punto a la lista de verificación final del documento:
