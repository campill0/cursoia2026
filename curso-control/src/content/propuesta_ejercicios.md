...
Cuando no haya datos suficientes, una buena respuesta no “rellena”:

debe decir “NO CONSTA / INFORMACIÓN INSUFICIENTE” o pedirte 1–2 datos concretos.
...
En un chat normal, escribe:

“A partir de ahora, respóndeme siempre en tabla y sin emojis.”

Haz 2 preguntas cualquiera y comprueba si lo respeta.

Abre un chat temporal y repite el punto 2 sin mencionar la instrucción.
...
Eres capaz de explicar con tus palabras: “qué se arrastra dentro de un chat” y “cómo se evita mezclar”.
...
Pide una tarea con un formato claro. Ejemplo:

“Resúmeme este texto en 5 bullets sin inventar y sin introducción.”

Si la respuesta sale mal (o tú provocas que salga mal), NO lo corrijas con otro mensaje.

En su lugar:

Edita tu prompt original, aclara la instrucción (más concreta) y vuelve a generar.

(Opcional) Crea una rama desde el punto original y prueba una estrategia diferente.

Entregable

Captura o copia de:

Resultado “antes” (deriva)

Resultado “después” (editado)

(si hay) resultado de la rama alternativa

Criterio de éxito

El “después” cumple el formato y no arrastra errores del hilo anterior.
...
Pide un texto largo (600–900 palabras) y que lo ponga en Canvas.

Haz 5 cambios locales (uno por uno):
...
En cada cambio, indica: qué parte y qué cambio exacto.

Entregable

Documento final + lista de 5 cambios (en 5 líneas).

Criterio de éxito

Cambia lo seleccionado y no “re-escribe” el resto.
...
Prepara un texto de 2.500–4.000 palabras (puede ser un artículo largo).

Formula 10 preguntas:
...
Responde con modo rápido (Instant o equivalente).

Responde con modo razonador (Thinking o equivalente).

Apunta fallos por tipo: “se lo inventa”, “se equivoca”, “se vuelve genérico”, “se contradice”.

Entregable

Tabla: Pregunta | Acierta? (Instant) | Acierta? (Thinking) | Observación

Criterio de éxito

Identificas un patrón: cuándo merece la pena cambiar de modo.
...
Para 5 preguntas de actualidad, exige:

“Usa búsqueda web obligatoria y devuelve evidencia.”

Para 5 preguntas atemporales, exige:

“Prohibido navegar; usa conocimiento general y razonamiento.”

En las 5 de actualidad, pide siempre:

Afirmación → Fuente → Confianza (alta/media/baja)
...
No hay afirmaciones “huérfanas” en actualidad: todo tiene fuente o “NO CONSTA”.
...
Crea el proyecto/GPT con esas instrucciones.

Prueba 3 entradas diferentes.

Entregable

Documento “SPEC del asistente” (1 página) + 3 pruebas (prompt+salida)

Criterio de éxito

Consistencia: se comporta igual con 3 inputs distintos.
...
Extracción fiel (literal, sin interpretar)

Resumen ejecutivo (interpretación útil, pero sin inventar)

Tabla de datos (si aplica)

Entregable

3 salidas etiquetadas: EXTRACCIÓN / RESUMEN / TABLA

Criterio de éxito

La extracción es fiel y el resumen no añade hechos no presentes.
...
Documento final + checklist de cumplimiento (5–10 puntos)

Criterio de éxito

Está listo para copiar y presentar sin re-trabajo masivo.
...
Diseña 3 plantillas de prompt (una por tarea).
...
Reutiliza cada plantilla 2 veces con contenidos distintos.

Entregable

3 plantillas + 6 ejecuciones (2 por plantilla)

Criterio de éxito

La plantilla funciona en contextos diferentes sin romperse.
...
Describe el fallo con una etiqueta:

“inventó datos”, “me dio la razón sin pensar”, “se contradijo”, “citó fuentes falsas”, etc.

Propón un parche mínimo del prompt (no reescribas todo).
...
12 fichas: Fallo | Causa probable | Parche | Resultado A | Resultado B

Criterio de éxito

El parche mejora la salida sin añadir “ruido” innecesario.
...
A: pega el texto tal cual y pide un resultado.
...
Compara inventos y precisión.

Entregable

Prompt A, Prompt B, y comparación en 10 líneas.

Criterio de éxito

En B baja el número de suposiciones y mejora la fidelidad.
...
A: pide una tarea sobre el documento sin defensas.
...
Comprueba si el modelo “muerde el anzuelo”.

Entregable

Evidencia A/B + nota: “¿obedeció al documento o al usuario?”

Criterio de éxito

En B ignora las órdenes internas del texto.
...
creativa (ej.: ideas de campaña)

operativa (ej.: checklist de proceso)

analítica (ej.: detectar contradicciones)
...
priming de dominio “sin personalidad”
...
3 comparaciones A/B + 3 justificaciones

Criterio de éxito

Tu elección produce mejor salida con menos relleno.
...
Evalúa si en B disminuyen inventos.

Entregable

Tabla: Pregunta | A inventa? | B inventa? | Qué preguntó / qué dijo

Criterio de éxito

En B no “rellena”; pregunta bien o reconoce falta de datos.
...
Material: 6 premisas falsas:

2 obvias, 2 sutiles, 2 emocionales (con urgencia o presión).

Pasos

A: planteas la premisa falsa y pides opinión.
...
Observa si corrige o si acompaña el error.

Entregable

6 pares A/B + nota: “¿corrige? ¿con evidencia? ¿sin halagos?”

Criterio de éxito

En B te corrige y no se alinea con tu error.
...
Pasos
Resuelve el mismo problema en 3 niveles:

Libre + autochequeo (mínimo)

Guiado (pasos sugeridos)

Protocolo (pasos obligatorios)
Compara calidad y “ruido”.

Entregable

3 salidas + comparación (pros/contras de cada nivel)

Criterio de éxito

Sabes elegir el nivel adecuado según el problema.
...
Evalúa: ¿lo podrías usar ya?

Entregable

A/B + nota: “¿copiable sin editar? ¿qué sobra?”

Criterio de éxito

B es un documento usable, no un texto bonito.
...
En cada iteración, exige:

“di qué cambiaste”

“no reescribas lo que ya está bien”

Entregable

v1, v2, v3 + lista de cambios por versión

Criterio de éxito

Se ve una mejora progresiva sin caos.
...
Versión “portable” (sin Canvas, sin features específicas).

Versión “ChatGPT específica” (si quieres, con herramientas).

Explica en 3 líneas qué cambia y por qué.

Entregable

10 prompts (5+5) + 5 explicaciones

Criterio de éxito

Sabes adaptar sin perder calidad.
...
Si un alumno hace los ejercicios “sin A/B” y sin bitácora, puede creer que aprende… pero solo está chateando. La comparación A/B + evidencia es lo que convierte esto en entrenamiento real.
...
002_patologias_de_los_modelos_d…
...
004_fase_c_contexto_curado_y_ge…
...
Abre un chat y elige Instant.

Pega un texto largo (puede ser un capítulo de una ley/ordenanza).

Sigue conversando 10–15 turnos añadiendo más texto/preguntas.

Repite en Thinking con el mismo material. (Thinking está pensado para tareas largas y coherencia.) 
...
Pega un documento largo (o 30–50 páginas).

Redacta 12 preguntas: 4 sobre el inicio, 4 sobre el medio, 4 sobre el final.

Obliga a contestar con “Cita textual breve + respuesta”.
...
Caso A (Adjunto): adjunta un PDF largo (ley/ordenanza). Pregunta algo específico con matices (excepciones, disposiciones transitorias, definiciones).

Caso B (Pegado): pega solo el artículo/capítulo relevante en texto plano y repite la pregunta.

Exige en ambos: “si falta, di NO CONSTA y dime qué parte necesitas”.
...
Ronda 1 (Mapa): “Sácame el índice/estructura: capítulos, artículos, títulos, definiciones y anexos”.

Ronda 2 (Triage): “Para esta pregunta, dime qué 5 secciones son relevantes y por qué”.

Ronda 3 (Curado): pega solo esas secciones y lanza el prompt final.
...
Adjunta 3–5 documentos.

Pregunta un caso que obligue a priorizar (jerarquía, definiciones, excepciones).
...
Prepara un “caso legal” con 20 hechos + 10 condiciones normativas.

Versión 20%: solo lo esencial (5 hechos + 5 artículos).

Versión 80%: mete todo (20 hechos + 10 artículos + anexos).

Mismas 8 preguntas en ambas.
...
Trabajas 15–20 turnos sobre un documento largo.

Pides: “Genera un resumen jerárquico de lo acordado + lista de decisiones + dudas abiertas”.

Abres nuevo chat y pegas ese resumen como priming.
...
Con el mismo PDF adjunto, formula 5 preguntas equivalentes con redacciones distintas.

Exige en todas: “cita textual + localizador”.
...
Elige un “paquete” grande (libro o muchas leyes).

Ejecuta el flujo G4 (triage→curado→respuesta) en ChatGPT.

Repite el flujo en un long-context externo si tienes acceso:
...
Primera pasada: pide “qué artículos necesito” (triage).

Segunda pasada

004_fase_c_contexto_curado_y_ge…

sos artículos (no el PDF entero) y resuelve el caso.
...
Sub-sección: Trazabilidad rápida con “citas” para revisión manual
...
Pide siempre que la salida tenga 2 bloqu
...
Lista numerada de afirmaciones (C1, C2, C3…).

Bloque 2 — Evidencias
Para cada afirmación, una fila con:
...
Pide: “Para cada evidencia, añade una ancla (8–12 palabras exactas del texto)”.

El alumno abre el PDF/Word y valida con Ctrl+F esa ancla.
...
Si vas a trabajar con normativa larga en ChatGPT, en tus materiales

004_fase_c_contexto_curado_y_ge…

esgos: truncamiento silencioso (por límite de ventana) y truncación por recuperación (adjuntos vía RAG). Estos ejercicios están diseñados para que el alumno los vea “en carne viva” y adopte un proceso profesional de triage + contexto curado + evidencias. 
...
Para “saltos” a long-context, hay documentación oficial de Google y
...
mina la necesidad de método: solo te da más margen.